...
const { allSelected: allStatusSelected, toggleAll: toggleAllStatus } = useMultiSelectToggle(statusFilterSelected, distinctStatusLabels);
...
const historyItemsFiltered = computed<DecoratedHistoryItem[]>(() => {
    ...
    return base.filter((item) => {
        ...
        const haystack = [item.fileName, item.userName, item.requestId, item.statusLabel, dateLabel].join(' ').toLowerCase();
                ...
                <div class="flex flex-col gap-3">
                    <div class="flex flex-wrap items-center justify-end gap-3 search-bar-wrapper">
                        <div class="p-input-icon-left search-input">
                            ...
                            <InputText v-model="historySearch" size="small" class="w-64" placeholder="Buscar..." />
                        </div>
                    </div>
                </div>
                    ...
                    <DataTable :value="historyPageItems" size="small" :loading="isHistoryLoading" responsiveLayout="scroll">
                        ...
                        <Column style="width: 5%">
                            <template #header>
                                <button
                                    type="button"
                                    :class="['sortable-header', { active: sortField === 'requestId' }]"
                                    @click="toggleSort('requestId')"
                                    aria-label="Ordenar por ID"
                                    :aria-sort="sortField === 'requestId' ? (sortOrder === 1 ? 'ascending' : 'descending') : 'none'"
                                >
                        ...
                        <Column style="width: 20%">
                            <template #header>
                                <div class="header-with-filter">
                                    <button
                                        type="button"
                                        :class="['sortable-header', { active: sortField === 'userName' }]"
                                        @click="toggleSort('userName')"
                                        aria-label="Ordenar por Usuário"
                                        :aria-sort="sortField === 'userName' ? (sortOrder === 1 ? 'ascending' : 'descending') : 'none'"
                                    >
                            ...
                            <template #body="{ data }">
                                <div class="flex items-center gap-3">
                                    <Avatar v-if="data.userAvatar !== ''" :image="data.userAvatar" size="large" shape="circle" />
                        ...
                        <Column style="width: 40%">
                            <template #header>
                                <div class="header-with-filter">
                                    <button
                                        type="button"
                                        :class="['sortable-header', { active: sortField === 'fileName' }]"
                                        @click="toggleSort('fileName')"
                                        aria-label="Ordenar por Arquivo"
                                        :aria-sort="sortField === 'fileName' ? (sortOrder === 1 ? 'ascending' : 'descending') : 'none'"
                                    >
                        ...
                        <Column style="width: 10%">
                            <template #header>
                                <button
                                    type="button"
                                    :class="['sortable-header', { active: sortField === 'timestamp' }]"
                                    @click="toggleSort('timestamp')"
                                    aria-label="Ordenar por Data/Hora"
                                    :aria-sort="sortField === 'timestamp' ? (sortOrder === 1 ? 'ascending' : 'descending') : 'none'"
                                >
                        ...
                        <Column style="width: 10%">
                            <template #header>
                                ...
                                <Popover ref="statusFilterPanel" style="min-width: 14rem" class="filter-panel">
                                    ...
                                    <MultiSelect v-model="statusFilterSelected" :options="distinctStatusLabels" optionLabel="label" optionValue="value" display="chip" placeholder="Qualquer" class="w-full" :show-toggle-all="false">
                        ...
                        <Column header="Ações" style="width: 5%; text-align: center">
                            <template #body="{ data }">
                                <div class="actions-menu-wrapper">
                                    <Button aria-label="Ações" icon="pi pi-ellipsis-v" class="p-button-rounded p-button-text actions-trigger" @click="toggleRowMenu($event, data.requestId)" />
                                    <Menu :ref="(el) => setActionsMenuRef(el, data.requestId)" :model="buildActionsModel(data)" popup />
                                </div>
                            </template>
                        </Column>
                        ...
                        <template #empty>
                            <Message severity="info" :closable="false">
                                {{ historySearchTerm ? 'Nenhum registro encontrado para esta busca.' : 'Nenhuma importação registrado para os filtros atuais.' }}
                            </Message>
                        </template>
                    </DataTable>
                </div>
                ...
                <div class="mt-6 history-pagination-grid">
                    ...
                    <div class="paginator-col">
                        <div class="paginator-wrapper">
                            <Paginator :rows="historyPageSize" :totalRecords="historyFilteredCount" :first="(historyPage - 1) * historyPageSize" @page="handleHistoryPageChange" />
                        </div>
                    </div>
...
.history-table :deep(.actions-menu-wrapper) {
    display: flex;
    justify-content: center;
}
...
.history-table :deep(.actions-trigger:hover) {
    background: var(--surface-200);
    color: var(--surface-900);
}
:root[class*='app-dark'] .history-table :deep(.actions-trigger:hover) {
...
/* Grid paginação */
.history-pagination-grid {
    display: grid;
    gap: 1rem;
    grid-template-columns: repeat(3, 1fr);
    align-items: center;
}
.history-pagination-grid .page-size-col {
    justify-self: start;
}
.history-pagination-grid .paginator-col {
    justify-self: center;
}
.history-pagination-grid .summary-col {
    justify-self: end;
    text-align: right;
}
.paginator-wrapper {
