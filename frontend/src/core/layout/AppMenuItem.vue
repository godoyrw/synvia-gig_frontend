...
function matchesRoute(menuItem) {
    if (!menuItem) return false;

    if (menuItem.to && route.path === menuItem.to) {
        return true;
    }

    if (menuItem.items) {
        return menuItem.items.some((child) => matchesRoute(child));
    }

    return false;
}

function activateIfMatchesRoute() {
    if (matchesRoute(props.item)) {
