...
router.get('/import-files/history', async (req, res) => {
  try {
    const clientId = getClientId(req);
    ...
    const entries = await readImportLogs(clientId, date);
    const filteredByOperation = entries.filter((entry) => entry.operation?.startsWith('IMPORT_FILE'));
    const filteredByLevel = levelFilter
      ? filteredByOperation.filter((entry) => entry.level === levelFilter)
      : filteredByOperation;

    const sorted = filteredByLevel.sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime());
    ...
    const items = paginated.map((entry) => {
      ...
      return {
        ...
        fileName: entry.file_original_name ?? 'arquivo.csv',
        fileSizeBytes: entry.file_size_bytes ?? 0,
        fileHash: entry.file_hash_sha256 ?? null,
        durationMs: entry.duration_ms ?? null,
        totalRows: Number(extra.rows_total ?? extra.totalRows ?? 0),
        errorRows: Number(extra.rows_errors ?? extra.errorRows ?? 0)
      };
    });
    ...
    console.error('Erro ao consultar histórico de importações', error);
    res.status(500).json({ ok: false, message: 'Não foi possível carregar o histórico de importações.' });
  }
});
